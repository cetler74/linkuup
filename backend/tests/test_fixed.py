"""
Fixed tests with proper database table creation and session override.
"""
import pytest
...
def client(db_session):
    """Create a test client with proper async database dependency override."""
...
class TestFixed:
    """Fixed tests with proper database setup."""
    ...
    def test_places_endpoint_pagination(self, client):
        """Test that places endpoint works with pagination."""
        response = client.get("/api/v1/places?limit=5&offset=0")
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
    
    def test_places_endpoint_search(self, client):
        """Test that places endpoint works with search parameters."""
        response = client.get("/api/v1/places?sector=beauty&booking_enabled=true")
        assert response.status_code == 200
        data = response.json()
        assert isinstance(data, list)
